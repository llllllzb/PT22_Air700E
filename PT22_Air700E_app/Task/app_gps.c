...
字段16：校验值
*/
static uint8_t gsv_gp_count = 0;
static uint8_t gsv_bd_count = 0;
...
//                }
                if (item->item_data[7][0] != 0)
                {
                    gpsinfo->gpsCn[gsv_gp_count++] = atoi(item->item_data[7]);
                }
                if (item->item_data[11][0] != 0)
                {
                    gpsinfo->gpsCn[gsv_gp_count++] = atoi(item->item_data[11]);
                }
                if (item->item_data[15][0] != 0)
                {
                    gpsinfo->gpsCn[gsv_gp_count++] = atoi(item->item_data[15]);
                }
                if (item->item_data[19][0] != 0)
                {
                    gpsinfo->gpsCn[gsv_gp_count++] = atoi(item->item_data[19]);
                }
                break;
            case 1:
...
//                }
                if (item->item_data[7][0] != 0)
                {
                    gpsinfo->beidouCn[gsv_bd_count++] = atoi(item->item_data[7]);
                }
                if (item->item_data[11][0] != 0)
                {
                    gpsinfo->beidouCn[gsv_bd_count++] = atoi(item->item_data[11]);
                }
                if (item->item_data[15][0] != 0)
                {
                    gpsinfo->beidouCn[gsv_bd_count++] = atoi(item->item_data[15]);
                }
                if (item->item_data[19][0] != 0)
                {
                    gpsinfo->beidouCn[gsv_bd_count++] = atoi(item->item_data[19]);
                }
                break;
            case 2:
...
void parse_TXT(GPSITEM *item)
{
	...
	if (((gpsvaild == 3 || gpsvaild == 7) && ephemeriscnt >= 8) || ephemeriscnt >= 22)
	{
...
<4> UTC日期，年<5>时区
*/
void parse_ZDA(GPSITEM *item)
{
	//开启这个数据解析的意图就是知道这1秒钟nmea数据的最后一包数据是在哪里
	//知道最后一包数据就可以这1秒的数据完整的存入gpsinfo里面
	gpsinfo_s *gpsinfo;
    gpsinfo = &gpsinfonow;
	addGpsInfo(gpsinfo);
...
static int8_t calculateDistanceOfPoint(void)
{
    ...
    distance = calculateTheDistanceBetweenTwoPonits(lastuploadgpsinfo.latitude,
               lastuploadgpsinfo.longtitude, gpsinfo->latitude, gpsinfo->longtitude);
...
void gpsUploadPointToServer(void)
{
    ...
    if (gpsRequestGet(GPS_REQUEST_ACC_CTL) == 0 || gpsRequestGet(GPS_REQUEST_123_CTL))
    {
        tick = 0;
        return;
    }

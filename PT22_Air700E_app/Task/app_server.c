...
void privateServerConnTask(void)
{
    ...
    switch (privateServConn.fsmstate)
    {
        ...
        case SERV_LOGIN_WAIT:
            ...
            break;
        case SERV_READY:
            if (privateServConn.heartbeattick % ((sysparam.heartbeatgap * 60) - 2) == 0)
            {
                queryBatVoltage();
                moduleGetCsq();
            }
            if (privateServConn.heartbeattick % (sysparam.heartbeatgap * 60) == 0)
            {
                privateServConn.heartbeattick = 0;
                if (timeOutId == -1)
                {
...
static void hiddenServerConnTask(void)
{
    ...
    switch (hiddenServConn.fsmstate)
    {
        ...
        case SERV_LOGIN_WAIT:
            ...
            break;
        case SERV_READY:
            if (hiddenServConn.heartbeattick % ((sysparam.heartbeatgap * 60) - 2) == 0)
            {
                queryBatVoltage();
                moduleGetCsq();
            }
            if (hiddenServConn.heartbeattick % (sysparam.heartbeatgap * 60) == 0)
            {
                hiddenServConn.heartbeattick = 0;
                if (timeOutId == -1)
                {
        ...
        case JT808_AUTHENTICATION:
            ...
            if (jt808ServConn.runTick % 60 == 0)
            {
                ...
                else
                {
                    ...
                    jt808ServConn.hbtTick = (sysparam.heartbeatgap * 60);
                    jt808RegisterLoginInfo(dynamicParam.jt808sn, dynamicParam.jt808isRegister, dynamicParam.jt808AuthCode, dynamicParam.jt808AuthLen);
            ...
            break;
        case JT808_NORMAL:
            if (++jt808ServConn.hbtTick >= (sysparam.heartbeatgap * 60))
            {
...
void agpsRequestClear(void)
{
	...
	tmos_memset(&agpsServConn, 0, sizeof(agps_connfsm_e));
    LogMessage(DEBUG_ALL, "agpsRequestClear==>OK");
}

/**************************************************
@bref       agps socket数据接收
@param
@return
@note
**************************************************/
...
static void agpsSocketRecv(char *data, uint16_t len)
{
    ...
    portUartSend(&usart1_ctl, data, len);
}
...
static void agpsServerChangeFsm(agps_connfsm_e fsm)
{
...
static void agpsServerConnTask(void)
{
    ...
    if (gpsinfo->fixstatus || sysinfo.gpsRequest == 0)
    {
    	if (gpsinfo->fixstatus)
    		LogPrintf(DEBUG_ALL, "gps has fix,close agps");
    	if (sysinfo.gpsRequest == 0)
    		LogPrintf(DEBUG_ALL, "gps has close,close agps");
        socketDel(AGPS_LINK);
        agpsRequestClear();
        return;
    }
    ...
    {
        LogPrintf(DEBUG_ALL, "wait agps server ready:%d", agpsServConn.runTick);
    ...
    switch (agpsServConn.fsm)
    {
    	case AGPS_INIT:
    		...
    		break;
    	case AGPS_WAIT:
    		LogPrintf(DEBUG_ALL, "sysTick:%d gpsUpdatetick:%d", sysinfo.sysTick, sysinfo.gpsUpdatetick);
			if (sysinfo.sysTick - sysinfo.gpsUpdatetick <= 1)
			{
				/* gps芯片正常启动 */
			...
			else
			{
				...
				{
					agpsServConn.errCnt++;
					if (agpsServConn.errCnt >= 3)
					{
						LogMessage(DEBUG_ALL, "agps server err, cancel agps");
						socketDel(AGPS_LINK);
        				agpsRequestClear();
        				agpsServerChangeFsm(AGPS_INIT);
					}
					...
					{
						LogPrintf(DEBUG_ALL, "agps server err:%d, try again", agpsServConn.errCnt);
						agpsServerChangeFsm(AGPS_INIT);
					}
				}
			}

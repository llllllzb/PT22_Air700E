...
static void doHbtInstruction(ITEM *item, char *message)
{
    ...
    {
        sprintf(message, "The time of the heartbeat interval is %d minutes;", sysparam.heartbeatgap);
    }
    else
    {
        ...
        paramSaveAll();
        sprintf(message, "Update the time of the heartbeat interval to %d minutes;", sysparam.heartbeatgap);
    }
}
...
void do123Instruction(ITEM *item, insMode_e mode, char *message, void *param)
{
	insParam_s *insparam;
	mode123 = mode;
	if (param != NULL)
...
void doAPNInstruction(ITEM *item, char *message)
{
    ...
    else
    {
        ...
        {
            strncpy((char *)sysparam.apn, item->item_data[1], 50);
        }
        if (item->item_data[2][0] != 0 && item->item_cnt >= 3)
        {
            strncpy((char *)sysparam.apnuser, item->item_data[2], 50);
        }
        if (item->item_data[3][0] != 0 && item->item_cnt >= 4)
        {
            strncpy((char *)sysparam.apnpassword, item->item_data[3], 50);
        }
...
static void doGpioSelInstruction(ITEM *item, char *message)
{
	if (item->item_data[1][0] == 0 || item->item_data[1][0] == '?')
	{
		sprintf(message, "Gpio sel is %s", sysparam.gpioSel > 0 ? "NTC" : "LDR");
	}
	else
	{
		...
		paramSaveAll();
		sprintf(message, "Update gpio sel to %s", sysparam.gpioSel > 0 ? "NTC" : "LDR");
	}
}
...
static void doStepMotionInstruction(ITEM *item, char *message)
{
    if (item->item_data[1][0] == 0 || item->item_data[1][0] == '?')
    {
		sprintf(message, "Gsensor motion step %d", sysparam.motionstep);    	
    }
    else
    {
		...
		paramSaveAll();
		sprintf(message, "Update gsensor motion step %d", sysparam.motionstep);
    }
}
...
static void doDeleteMsgInstruction(ITEM *item, char *message)
{
	deleteAllMessage();
...
static void doinstruction(int16_t cmdid, ITEM *item, insMode_e mode, void *param)
{
    ...
    switch (cmdid)
    {
        ...
        case MODE_INS:
            ...
            break;
        case POSITION_INS:
            do123Instruction(item, mode, message, param);
            break;
        case APN_INS:
        ...
        case BATSEL_INS:
        	...
        	break;
        case GPIOSEL_INS:
			doGpioSelInstruction(item, message);
        	break;
        case STEPMOTION_INS:

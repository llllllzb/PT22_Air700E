...
static int protocolGPSpack(gpsinfo_s *gpsinfo, char *dest, int protocol, gpsRestore_s *gpsres)
{
    ...
    unsigned long lo;
    double f_la, f_lo;
    unsigned char speed, gps_viewstar = 0, beidou_viewstar = 0;
    int direc;
    uint16_t year;
    uint8_t  month;
    uint8_t date;
...
int createProtocol13(unsigned short Serial, char *DestBuf)
{
    ...
    uint16_t value;
    gpsinfo_s *gpsinfo;
    uint8_t gpsvewstar = 0, beidouviewstar = 0;
...
void protocolInfoResiter(uint8_t batLevel, float vol, uint16_t sCnt, uint16_t runCnt)
{
    ...
    LogPrintf(DEBUG_ALL, "Register bat:%d, Vol:%.2f, startUp:%d, runCnt:%d", protocolInfo.batLevel,
							    											 protocolInfo.Voltage,
							    											 protocolInfo.startUpcnt,
							    											 protocolInfo.runCnt);
}

...
void GPIOA_IRQHandler(void)
{
	...
	if (iqr & SOS_KEY_PIN)
	{
		...
		GPIOA_ClearITFlagBit(SOS_KEY_PIN);
	}
...
void portModuleGpioCfg(uint8_t state)
{
    if (state)
    {
        ...
        GPIOB_ModeCfg(DTR_PIN, GPIO_ModeOut_PP_5mA);
        GPIOA_ModeCfg(RST_PIN, GPIO_ModeOut_PP_5mA);
        GPIOB_ModeCfg(RING_PIN, GPIO_ModeIN_Floating);
        GPIOB_ITModeCfg(RING_PIN, GPIO_ITMode_FallEdge);
...
void portGsensorCtl(uint8_t onoff)
{
    ...
    if (onoff)
    {
		...
		PWR_PeriphWakeUpCfg( ENABLE, RB_SLP_GPIO_WAKE, Long_Delay );
        GPIOA_ModeCfg(GSINT_PIN, GPIO_ModeIN_Floating);
        GPIOA_ITModeCfg(GSINT_PIN, GPIO_ITMode_RiseEdge);
        if (GPIOA_ReadPortPin(GSINT_PIN)) 
		{
        	GPIOA_ResetBits(GSINT_PIN);
	    }
    ...
    else
    {
        ...
        GSPWR_OFF;
        GPIOA_ModeCfg(GSINT_PIN, GPIO_ModeIN_PD);
        R16_PA_INT_EN &= ~GSINT_PIN;
        motionClear();
        terminalAccoff();
